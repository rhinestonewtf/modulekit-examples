...
contract AutoSavings is ConditionalExecutor, ISessionKeyValidationModule {
     ...
     * @dev  This SessionKey Validator only allows transaction to itself, not to other
     * modules/contracts
    ...
    function validateSessionUserOp(
        ...
        returns (bool)
    {
        // ensure that only this contract is called. No other external contracts shall be
        // authorized.
        ...
        TokenTxEvent memory tokenTxEvent;
        {
            ...
            // ensure that trigger(spendToken,,,,) is the same token as specified in the
            // TokenTxEvent
            ...
            // ensure that TokenTxEvent is an event where the account owner actually received
            // tokens.
        ...
        address[] memory recoveredSigners = CheckSignatures.recoverNSignatures({
            ...
            requiredSignatures: authorizedRelayRecord.threshold // threshold of signatures that are
                // required according to user
         });

        // ensure that recovered signers are unique. Malicious Actor could sign multiple times with
        // same address
