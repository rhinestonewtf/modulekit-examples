...
contract FlashloanLender is IFallbackMethod, IExecutor {
        ...
        returns (bool)
    {
        ...
        console2.log("this (lender)", address(this));
        (FlashLoanType flashLoanType,,) = abi.decode(data, (FlashLoanType, bytes, bytes));

        if (flashLoanType == FlashLoanType.ERC721) {
            IMSA(account).executeFromExecutor({
                target: address(token),
                value: 0,
                callData: abi.encodeCall(
                    IERC721.transferFrom, (address(account), address(borrower), value)
                    )
            });
        }
        // TODO impl ERC20
